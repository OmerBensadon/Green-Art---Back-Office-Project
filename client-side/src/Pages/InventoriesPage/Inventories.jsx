...
const url =
  "http://194.90.158.74/cgroup96/prod/api/inventoryItems/get?timestamp=" +
  Date.now();

const urlpost = "http://194.90.158.74/cgroup96/prod/api/inventoryItems/post";
const urldelete =
  "http://194.90.158.74/cgroup96/prod/api/inventoryItems/delete";
  ...
  const columns = [
    {
      ...
      right: true,
      cell: (row) => (
        <Button onClick={() => deleteInventoryItem(row.itemSerialNum)}>
          מחיקת פריט
        </Button>
      ),
    },
  ...
  return (
    <div>
      <div className="headerCatalog">
        ...
        <h1 className="h1Catalog">מלאי</h1>
      </div>
      <InventoryPopUp
        trigger={buttonPopUp}
